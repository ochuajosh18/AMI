...
$(document).ready(function() {
	let LCLDB_USER, LCLDB_CUSTOMER, LCLDB_SALESPERSON, LCLDB_ROLES, LCLDB_MAINCUSTOMERS, ROLE_IDS = role_localdata;
	let DT_USER, DT_CUSTOMER, DT_SALESPERSON, DT_SALESPERSON_CUSTOMERS, DT_CURRENTSALESPERSON_CUSTOMERS, DT_ALLSALESPERSON_CUSTOMERS;
	...
	console.log(LCLDB_MAINCUSTOMERS)
	...
	function updateUserLocal(userId, { firstName, lastName, department, roleId }) {
		...
		let index2 = LCLDB_ROLES.findIndex(item => item.id == roleId);
		LCLDB_USER[index]['role']     =  LCLDB_ROLES[index2].role;
	}
			...
			setTimeout(function() {
				if (userType == 'customer') {
					...
					let doc = {
						...
						lastPassUpdate : moment().toISOString(),
						lastWrongLogin : moment().toISOString()
					}
	...
	$('#btn-help-multiple').attr('data-content', 
		`<a><h6 id="btn-add-user" data-toggle="modal" data-target="#modal-default" data-help="user" style="cursor: pointer;">How to add user account</h6></a>
		<a><h6 id="btn-edit-user" data-toggle="modal" data-target="#modal-default" data-help="user" style="cursor: pointer;">How to edit user account</h6></a>
		<a><h6 id="btn-delete-user" data-toggle="modal" data-target="#modal-default" data-help="user" style="cursor: pointer;">How to delete user account</h6>
		<a><h6 id="btn-edit-distributionchannel" data-toggle="modal" data-target="#modal-default" data-help="user" style="cursor: pointer;">How to edit customer account distribution channel</h6>
		<a><h6 id="btn-resend-customerinvite" data-toggle="modal" data-target="#modal-default" data-help="user" style="cursor: pointer;">How to resend customer invitation</h6>
		<a><h6 id="btn-map-customer" data-toggle="modal" data-target="#modal-default" data-help="user" style="cursor: pointer;">How to map customer to salesperson account</h6>
		<a><h6 id="btn-resend-salespersoninvite" data-toggle="modal" data-target="#modal-default" data-help="user" style="cursor: pointer;">How to resend salesperson invitation</h6></a>`
	);

	$(document).on("click", "#btn-add-user", function(event) {
		let helptype = $('#btn-add-user').attr('data-help');
		helpCarouselMultiple(helptype, 'add')
    });

    $(document).on("click", "#btn-edit-user", function(event) {
		let helptype = $('#btn-edit-user').attr('data-help');
		helpCarouselMultiple(helptype, 'edit')
    });

    $(document).on("click", "#btn-delete-user", function(event) {
		let helptype = $('#btn-delete-user').attr('data-help');
		helpCarouselMultiple(helptype, 'delete')
    });

    $(document).on("click", "#btn-edit-distributionchannel", function(event) {
    	let helptype = $('#btn-edit-distributionchannel').attr('data-help');
    	helpCarouselMultiple(helptype, 'channel')
    });

    $(document).on("click", "#btn-resend-customerinvite", function(event) {
    	let helptype = $('#btn-resend-customerinvite').attr('data-help');
    	helpCarouselMultiple(helptype, 'resendcustomerinvite')
    });

    $(document).on("click", "#btn-map-customer", function(event) {
    	let helptype = $('#btn-map-customer').attr('data-help');
    	helpCarouselMultiple(helptype, 'mapcustomer')
    });

    $(document).on("click", "#btn-resend-salespersoninvite", function(event) {
    	let helptype = $('#btn-resend-salespersoninvite').attr('data-help');
    	helpCarouselMultiple(helptype, 'resendsalespersoninvite')
    });
	...
	$('#add-user-btn').click(function(){
		...
		setTimeout(() => {
			...
			doc.role = '',
			doc.lastPassUpdate = moment().toISOString(),
			doc.lastWrongLogin = moment().toISOString(),
			...
			updateUser2(user.id, changes, (err, res) => {
				...
				if (res.statusCode <= 299) {
					...
					if (LOG_FUNCTION) {
						createLog(`LOG::${LOCAL_STORAGE.referenceId}`, {
							dateCreated : moment().toISOString(),
							action : `Edited user ${user.email} as ${changes.firstName} ${changes.lastName}`,
							module : "User Management/User",
							app : "AMI"
						}, moment().toISOString());
					}
	...
	$('#delete-user-btn').click(function(){
		...
		let oldUser = LCLDB_USER.find(item => item.id == user.id);
			...
			deleteUser(user.id, (err, res) => {
				...
				if (res.statusCode <= 299) {
					...
					if (LOG_FUNCTION) {
						createLog(`LOG::${LOCAL_STORAGE.referenceId}`, {
							dateCreated : moment().toISOString(),
							action : `Deleted user ${oldUser.email}`,
							module : "User Management/User",
							app : "AMI"
						}, moment().toISOString());
					}
	...
	$('#edit-distribution-channel-btn').click(function(){
		...
		setTimeout(() => {
			let id = customer.id;
			let oldCust = LCLDB_CUSTOMER.find(item => item.id == id);
			...
			updateUser2(id, doc, (err, res) => {
				...
				if (res.statusCode <= 299) {
					...
					if (LOG_FUNCTION) {
						createLog(`LOG::${LOCAL_STORAGE.referenceId}`, {
							dateCreated : moment().toISOString(),
							action : `Edited customer ${oldCust.customerCode}'s distribution channel from ${oldCust.distributionChannel=='01' ? 'Normal Sales' : 'Direct Sales'} to ${doc.distributionChannel=='01' ? 'Normal Sales' : 'Direct Sales'}`,
							module : "User Management/User",
							app : "AMI"
						}, moment().toISOString());
					}
