...
$(document).ready(function() {
	...
	function loadOrderTable() {
		if (config.db == 'couchbase') {
			loadSpecialOrder2(function(err, res) {
				try {
				...
				if (DT_CHECK_NORMAL_ORDER.rows().count() > 0) { // there is normal order
					...
					if (DEPLOYED_ON == 'LOCAL' || DEPLOYED_ON == 'DEV') {
						...
						if (proceedtoSAP) { // order go to SAP
							...
							if (creditExceed < 5000 && overduePayment < 5000) {
								toEmails = [].concat(BSTSG_EMAILS.salesManager).concat(['matsukawa.yang@bridgestone.com']);
							} else if (creditExceed >= 5000 || overduePayment >= 5000) {
								toEmails = [].concat(BSTSG_EMAILS.accountTeam).concat(['matsukawa.yang@bridgestone.com']);
							}

							ccEmails = [].concat(BSTSG_EMAILS.logisticTeam);
							// TO  • Sales / • Account
							// CC • Logistic
						}
					}
