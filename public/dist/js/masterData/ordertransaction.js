$(document).ready(function() {
	...
	$('#print-sales-form-btn, #block-invoice-btn, #unblock-invoice-btn').hide();
		...
		if (config.db == 'couchbase') {
			if (LOCAL_STORAGE.role != 'CUSTOMER') {
				loadAllOrderBackorder({startDate, endDate}, (err, res) => {
					...
					else if (res.statusCode <= 299) { LCLDB_ORDER = res.result; data = LCLDB_ORDER; console.log(LCLDB_ORDER) }
				});
			} else {
	...
	function showBlockInvoice(isInvoiceBlocked, orderStatus) {
		$('#block-invoice-btn, #unblock-invoice-btn').hide();
		if(isInvoiceBlocked == 'true')  {
			$('#unblock-invoice-btn').fadeIn();
		} else if(isInvoiceBlocked != 'true' && orderStatus == 'invoiced') {
			$('#block-invoice-btn').fadeIn();
		}
	}
	...
	function loadOrderDetailsTable(salesOrderNo, mode) {
		...
		let isInvoiceBlocked = orders[0].invoiceBlocked;
		...
		if (DEPLOYED_ON != 'LOCAL') {
			if(orderStatus == 'confirmed' || orderStatus == 'scheduled' || orderStatus == 'delivered') {
				...
				showBlockInvoice(isInvoiceBlocked, 'invoiced');
			}
		} else {
			showBlockInvoice(isInvoiceBlocked, orderStatus);
		}
	...
	$('#block-invoice-btn').click(function(){
		let data = DT_ORDER.rows().data().toArray();
		let salesOrderNo = data[0].salesOrderNo;
		...
		setTimeout(() => {
			...
			updateOrders(changesObj, (err, res) => {
				...
				if (err || res.statusCode >= 300) {
					...
					resultNotify('fa fa-times', 'ERROR', 'Blocking invoice failed.<br>Something went wrong. Please try again later', 'danger');
					return;
				}

				if (res.statusCode <= 299) {
					resultNotify('fa-check-circle', 'SUCCESS', 'Invoice successfully blocked', 'success');
					config.db = 'local';
					updateInvoiceLocal(changesObj, LCLDB_ORDER);

					if (LOG_FUNCTION) {
						createLog(`LOG::${LOCAL_STORAGE.referenceId}`, {
							dateCreated : moment().toISOString(),
							action : `Blocked ${salesOrderNo}'s invoice`,
							module : "Order Transaction/Manage Invoice",
							app : "AMI"
						}, moment().toISOString());
					}
					setTimeout(function(){ loadOrderDetailsTable(salesOrderNo, 'details'); }, 2000);
				}
			});
	...
	$('#unblock-invoice-btn').click(function(){
		let data = DT_ORDER.rows().data().toArray();
		let salesOrderNo = data[0].salesOrderNo;
		...
		setTimeout(() => {
			...
			updateOrders(changesObj, (err, res) => {
				...
				if (err || res.statusCode >= 300) {
					console.log(res);
					$('.loading-state').fadeOut('slow');
					resultNotify('fa fa-times', 'ERROR', 'Unblocking invoice failed.<br>Something went wrong. Please try again later', 'danger');
					return;
				}

				if (res.statusCode <= 299) {
					resultNotify('fa-check-circle', 'SUCCESS', 'Invoice successfully unblocked', 'success');
					config.db = 'local';
					updateInvoiceLocal(changesObj, LCLDB_ORDER);
					...
					if (LOG_FUNCTION) {
						createLog(`LOG::${LOCAL_STORAGE.referenceId}`, {
							dateCreated : moment().toISOString(),
							action : `Unblocked ${salesOrderNo}'s invoice`,
							module : "Order Transaction/Manage Invoice",
							app : "AMI"
						}, moment().toISOString());
					}
	...
	// update local db copy - invoiceBlocked
	function updateInvoiceLocal(order, localdb) {
		for (let i in order) {
			let id = i;
			let index = localdb.findIndex(item => item.id == id);
			localdb[index].invoiceBlocked = order[i].invoiceBlocked;
		}
	}
});